...
- {Title/description of the internal document — e.g. "Q3 2026 Vendor Evaluation Memo (internal wiki)"}
  {One line: what it covers, when to cite it}
...
- **Internal sources get their own section**, separate from `Knowledge` — company-internal docs, wikis, or prior reports the domain expert provides directly. Unlike `Knowledge` entries, these can't be independently verified by the agent (no public access to check them against); they're taken on the expert's word alone. A link is optional — many internal sources aren't fetchable at all, and it's fine to cite one by name and description only.
